 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
	class XTS_Import extends WP_Importer {
		...
		function import_end() {
			...
			$this->imported_data['theme_name'] = $theme_slug;
			update_option( 'xts_imported_data', $this->imported_data );
	...
	<?php
			...
			foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
				...
				if ( ! empty( $_POST['user_map'][$i] ) ) {
					...
					if ( ! is_wp_error( $user_id ) ) {
						...
						if ( defined('XTS_IMPORT_DEBUG') && XTS_IMPORT_DEBUG )
			...
			foreach ( $this->categories as $cat ) {
				...
				if ( ! is_wp_error( $id ) ) {
					if ( isset($cat['term_id']) ) {
						...
						$this->imported_data['categories'][] = $id;
					}
				} else {
			...
			foreach ( $this->tags as $tag ) {
				...
				if ( ! is_wp_error( $id ) ) {
					if ( isset($tag['term_id']) ){
						...
						$this->imported_data['tags'][] = $id['term_id'];
					}
				} else {
			...
			foreach ( $this->terms as $term ) {
				...
				if ( ! is_wp_error( $id ) ) {
					if ( isset($term['term_id']) ){
						...
						$this->imported_data['terms'][$id['term_id']] = $term['term_taxonomy'];
					}
				} else {
			...
			foreach ( $this->posts as $post ) {
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					$postdata = array(
						...
						'post_date_gmt' => $post['post_date_gmt'], 'post_content' => $post_content,
		...
		function process_menu_item( $item ) {
			...
			$this->imported_data['menus'] = array_unique( array_merge( array( $menu_id ), $this->imported_data['menus'] ) );
		...
		function backfill_attachment_urls() {
			...
			foreach ( $this->url_remap as $from_url => $to_url ) {
				...
				$replace_meta_query = $wpdb->prepare(
					"UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key IN ( 'enclosure' )",
					$from_url,
					$to_url
				);

				$wpdb->query( $replace_meta_query );

				$replace_meta_query = $wpdb->prepare(
					"UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key = '_elementor_data'",
					str_replace('/','\\\\\\/', $from_url ),
					str_replace('/','\\\\\\/', $to_url )
				);
