					...
					<?php foreach ( $element_args['gallery'] as $key => $image ) : ?>
						<div class="xts-col">
							<div class="xts-gallery-thumb">
								<?php echo apply_filters( 'xts_image', '<img src="' . esc_url( xts_get_image_url( $image['id'], 'gallery_thumbs_image', $element_args ) ) . '" alt="' . esc_attr( Control_Media::get_image_alt( $image ) ) . '">' ); // phpcs:ignore ?>
							</div>
						</div>
					<?php endforeach; ?>
				</div>
