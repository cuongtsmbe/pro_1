...
if ( isset( $quick_shop ) ) {
	$form_classes .= ' xts-variations_form';
} else {
	$form_classes .= ' variations_form';

	if ( xts_get_opt( 'single_product_variations_price' ) ) {
		xts_enqueue_js_script( 'variations-price' );
	}
}
$id = uniqid();
// XTS Extra.
		...
		<table class="variations">
			<tbody>
				<?php foreach ( $attributes as $attribute_name => $options ) : ?>
					<tr>
						<th class="label cell"><label for="<?php echo esc_attr( sanitize_title( $attribute_name ) . '_' . $id ); ?>"><?php echo wc_attribute_label( $attribute_name ); // WPCS: XSS ok. ?></label></th>
						<td class="value cell">
							<?php
								wc_dropdown_variation_attribute_options(
									array(
										...
										'product'   => $product,
										'id'        => sanitize_title( $attribute_name ) . '_' . $id
									)
								);
