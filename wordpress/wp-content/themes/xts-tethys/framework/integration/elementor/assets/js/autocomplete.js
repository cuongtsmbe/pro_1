jQuery(window).on('elementor:init', function() {
	var postSearch = elementor.modules.controls.BaseData.extend({
		...
		onReady: function() {
			...
			if (this.ui.select.attr('multiple')) {
				this.ui.select.on('select2:select', function (e) {
					var id = e.params.data.id;
					var $elm = jQuery(e.target).children('[value='+id+']');
					self.ui.select.append($elm);
					self.ui.select.trigger('change.select2');
				});
			}
	...
	elementor.addControlView('xts_autocomplete', postSearch);
});
