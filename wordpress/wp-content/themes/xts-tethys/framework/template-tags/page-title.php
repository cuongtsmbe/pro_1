...
if ( ! function_exists( 'xts_page_title' ) ) {
	...
	function xts_page_title() {
		...
		if ( ( ! is_singular( 'xts-portfolio' ) || ( is_singular( 'xts-portfolio' ) && 'page-title' !== $portfolio_single_design ) ) && ( ! is_singular( 'post' ) || ( is_singular( 'post' ) && 'page-title' !== $blog_single_design ) ) ) {
		...
		if ( is_singular( 'post' ) || xts_is_blog_archive() ) {
			...
			if ( xts_get_opt( 'page_title_show_current_post_title' ) && is_singular( 'post' ) ) {
				$title = get_the_title();
			}
		...
		if ( 'page-title' === $blog_single_design && is_singular( 'post' ) ) {
			...
			?>
				<div class="xts-page-title<?php echo esc_attr( $wrapper_classes ); ?>">
					...
					<div class="container">
						<?php do_action( 'xts_before_post_page_title' ); ?>
						...
						<?php do_action( 'xts_after_post_page_title' ); ?>
					</div>
				</div>
			<?php
		...
		if ( 'page-title' === $portfolio_single_design && is_singular( 'xts-portfolio' ) ) {
			...
			?>
				<div class="xts-page-title<?php echo esc_attr( $wrapper_classes ); ?>">
					<div class="xts-page-title-overlay xts-fill" style="<?php echo esc_attr( $custom_styles ); ?>"></div>
					<div class="container">
						<?php do_action( 'xts_before_portfolio_page_title' ); ?>
						...
						<?php do_action( 'xts_after_portfolio_page_title' ); ?>
					</div>
				</div>
			<?php
			return;
		...
		// Heading for vendor page.
		if ( xts_is_dokan_installed() && dokan_is_store_page() ) {
			$title = dokan()->vendor->get( get_query_var( 'author' ) )->get_shop_name();
		}
		...
		if ( ( is_singular( 'page' ) && ( ! $page_for_posts || ! is_page( $page_for_posts ) ) ) || ! is_singular( 'product' ) ) {
			?>
				<div class="xts-page-title<?php echo esc_attr( $wrapper_classes ); ?>">
					<div class="xts-page-title-overlay xts-fill" style="<?php echo esc_attr( $custom_styles ); ?>"></div>
					<div class="container">
						<?php do_action( 'xts_before_page_title' ); ?>
						...
						<?php do_action( 'xts_after_page_title' ); ?>
					</div>
				</div>
			<?php
...
if ( ! function_exists( 'xts_breadcrumbs' ) ) { // phpcs:disable
	...
	function xts_breadcrumbs() {
		...
		$show_current_post = xts_get_opt( 'page_title_breadcrumbs_show_current_post', 1 ); // 1 - show current post.
...
if ( ! function_exists( 'xts_current_shop_breadcrumbs' ) ) {
	 ...
	 *
	 * @param bool $return Is return template.
	 *
	 * @return string
	 * @since 1.0.0
	 */
	function xts_current_shop_breadcrumbs( $return = false ) {
		if ( $return ) {
			ob_start();
		}
