...
class Patch {
	...
	/**
	 * Flag successfully installed patches.
	 *
	...
	public function apply() {
		...
		if ( ! isset( $patches_map[ $this->patch_id ] ) ) {
			wp_send_json(
				array(
					'message' => esc_html__( 'Patch with this ID does\'t exist.', 'xts-theme' ),
					'status'  => 'error',
				)
			);
		}
		...
		if ( $this->successful_write_files ) {
			...
			$patch_success[ XTS_FRAMEWORK_VERSION ][ $this->patch_id ] = true;

			update_option( 'xts_successfully_installed_patches', $patch_success );

			wp_send_json(
				array(
					'message' => esc_html__( 'Patch has been successfully applied.', 'xts-theme' ),
					'status'  => 'success',
				)
			);
		}
		...
		wp_send_json(
			array(
				'message' => esc_html__( 'Something went wrong during patch installation. Patch can\'t be applied. Please, try again later.', 'xts-theme' ),
				'status'  => 'error',
			)
		);
	}
